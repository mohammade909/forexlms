...
const SectionAttendance = () => {
  ...
  const { sections, loading: sectionLoading } = useSelector(
  ...
  useEffect(() => {
    if (selectedClass) {
      dispatch(fetchSectionByClass(selectedClass));
  ...
  const handleAttendanceSubmit = ({ class_id, section_id, reason }) => {
    ...
    console.log("Attendance Record Submitted: ", attendanceRecord);
  ...
  return (
    <div className="attendance-container p-4 text-[12px] font-normal">
      ...
      <div className="flex flex-wrap items-center mb-4 gap-4">
        ...
        {selectedClass && (
          <div className="flex items-center space-x-2">
            <label className="text-gray-600">Select Section:</label>
            <select
